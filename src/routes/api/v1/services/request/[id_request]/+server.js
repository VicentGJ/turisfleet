...
export async function GET({ params }) {
    ...
    if (result.length == 0) throw error(404, { 'message': `Request with id ${id_request} not found` })
...
export async function PUT({ params, request }) {
    ...
    const result = await sequelize.transaction(async (t) => {
        await sequelize.query(
            `
            UPDATE ${table}
            SET id_driver=:id_driver,id_car=:id_car,id_copilot=:id_copilot
    ...
    if (result.length === 0) throw new error(404, { message: `Request with id ${identifier} not found` })
    return json(result[0]);
}